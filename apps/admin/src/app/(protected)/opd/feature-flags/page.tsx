    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-semibold uppercase tracking-wide text-slate-700">OPD Flags</h2>
          <div className="flex gap-2">
            <button onClick={() => tenantId && void loadFlags(tenantId)} className="rounded-md border border-slate-300 px-3 py-2 text-sm text-slate-700">
              Refresh
            </button>
            <button onClick={() => void seedModuleFlag()} disabled={!tenantId || savingKey === 'module.opd'} className="rounded-md border border-blue-300 px-3 py-2 text-sm text-blue-700 disabled:opacity-60">
            ...
            {opdFlags.map((flag) => (
              <div key={flag.key} className="flex flex-col gap-3 rounded-lg border border-slate-200 bg-slate-50 p-4 md:flex-row md:items-center md:justify-between">
                ...
                <button
                  ...
                  className={`inline-flex min-w-28 items-center justify-center rounded-md px-3 py-2 text-sm font-medium ${flag.enabled ? 'bg-emerald-600 text-white' : 'bg-slate-200 text-slate-800'} disabled:opacity-60`}
                >
