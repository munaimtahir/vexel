...
export async function main() {
    ...
    create: {
      ...
      rolePermissions: {
        create: [
          ...
          'opd.clinical_note.write', 'opd.clinical_note.sign',
          'opd.prescription.write', 'opd.prescription.publish',
          'opd.billing.read', 'opd.billing.manage', 'opd.document.read', 'opd.document.generate', 'opd.document.publish',
          'document.generate',
          ...SELF_SERVICE_PERMISSIONS,
        ].map((p) => ({ permission: p })),
      },
  ...
  const blueprints = [
    ...
    {
      id: 'bp-lipid-profile-v1',
      code: 'lipid_profile_visual_v1',
      name: 'Lipid Profile — Graphical Scale Report',
      templateFamily: 'GRAPHICAL_SCALE_REPORT',
      schemaType: 'TABULAR',
      isActive: true,
      sortOrder: 3,
      defaultConfigJson: {
        ...
        parameters: [
          ...
          },
          {
            ...
            sourceMatch: 'LDL',
            bands: [
              { label: 'Optimal', min: null, max: 100, colorToken: 'GOOD' },
              { label: 'Near Optimal', min: 100, max: 130, colorToken: 'INFO' },
              { label: 'Borderline High', min: 130, max: 160, colorToken: 'CAUTION' },
              { label: 'High', min: 160, max: 190, colorToken: 'BAD' },
              { label: 'Very High', min: 190, max: null, colorToken: 'BAD' },
            ],
          },
          {
            key: 'hdl',
          ...
          },
          {
            ...
            unit: 'mg/dL',
            sourceMode: 'parameter_name_match',
            sourceMatch: 'Triglycerides',
            bands: [
              { label: 'Normal', min: null, max: 150, colorToken: 'GOOD' },
              { label: 'Borderline High', min: 150, max: 200, colorToken: 'CAUTION' },
              { label: 'High', min: 200, max: 500, colorToken: 'BAD' },
              { label: 'Very High', min: 500, max: null, colorToken: 'BAD' },
            ],
          },
          {
            key: 'non_hdl',
            ...
            skipIfMissing: true,
            bands: [
              { label: 'Optimal', min: null, max: 130, colorToken: 'GOOD' },
              { label: 'Borderline High', min: 130, max: 160, colorToken: 'CAUTION' },
              { label: 'High', min: 160, max: null, colorToken: 'BAD' },
            ],
          },
          {
            key: 'tc_hdl_ratio',
            ...
            skipIfMissing: true,
            bands: [
              { label: 'Low Risk', min: null, max: 4, colorToken: 'GOOD' },
              { label: 'Moderate Risk', min: 4, max: 5, colorToken: 'CAUTION' },
              { label: 'High Risk', min: 5, max: null, colorToken: 'BAD' },
            ],
          },
        ],
      },
