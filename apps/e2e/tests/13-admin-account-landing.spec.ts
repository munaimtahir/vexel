...
test.describe('Admin Landing and Navigation', () => {
  test('limited user (catalog.read) lands on /admin/catalog and sees no admin-only sections', async ({ page }) => {
    await page.goto('/admin/login');
    await page.getByLabel('Email').fill(LIMITED_EMAIL);
    await page.getByLabel('Password').fill(LIMITED_PASSWORD);
    await page.getByRole('button', { name: /sign in/i }).click();

    // operator@demo.vexel.pk has catalog.read → resolveAdminLanding returns /catalog (not /account)
    await page.waitForURL('**/admin/catalog', { timeout: 15_000 });
