  ...
  return (
    <div>
        ...
        <div style={{ background: 'white', borderRadius: '8px', border: '1px solid #e2e8f0', padding: '32px', textAlign: 'center' }}>
          ...
          {encounter.status === 'resulted' && (
            <p style={{ margin: '8px 0 0', color: '#64748b', fontSize: '14px' }}>
              <Link href={`/encounters/${id}/verify`} style={{ color: '#2563eb' }}>Verify the encounter</Link> to generate reports.
            </p>
          )}
        </div>
      )}
      ...
      <div style={{ marginTop: '8px' }}>
        <Link href="/worklist" style={{ color: '#64748b', fontSize: '14px', textDecoration: 'none' }}>← Back to Worklist</Link>
      </div>
    </div>
  );
}
