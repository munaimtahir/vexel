...
| G3 | **Clean-server setup, rollback and restore proof.** Start from an empty server; rotate secrets; roll back; restore from backup on a fresh machine. | PASS for restore targets — current-server destructive restore with a safety snapshot passed, and the same package was restored into a disposable PostgreSQL target with manifest, MinIO archive, and counts verified. A full new-VPS/DNS cutover was not exercised. |
